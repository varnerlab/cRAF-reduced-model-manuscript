...
\subsubsection*{Gene expression model equations.}
...
For each gene $j=1,2,\dots,\mathcal{G}$, we modeled both the mRNA ($m_{j}$) and protein ($p_{j}$) abundance:
\begin{eqnarray}
...
In this study, the integration rule governing gene expression was the weighted fraction of promoter configurations that resulted in gene expression [REFHERE]:
\begin{equation}
...
A defining feature of ATRA-induced differentiation is G1/G0 cell cycle arrest followed by differentiation.
While we included E2F transcription, we do not consider a detailed model of restriction point regulation.
Instead, we formulated an effective model of the fraction of cells undergoing ATRA-induced cell cycle arrest, $\mathcal{A}$ as:
\begin{equation}\label{eq:a-eqn}
	\frac{d\mathcal{A}}{dt} = \epsilon_{1}\left(\mathrm{E2F}\right)^{\epsilon_{2}}
\end{equation}
The parameters $\epsilon_{1}$ and $\epsilon_{2}$ were estimated from experimental cell distribution measurements by \texttt{log} transforming Eq. \eqref{eq:a-eqn}:
\begin{eqnarray}
...
and $\dot{\mathcal{A}}_{ij}$ denotes the time derivative of the fraction of arrested cells (estimated from data).
...
the half-life of a typical mRNA and protein, and typical values for the copies per cell of RNA polymerase and ribosomes from literature (Table ZZ).
...
\begin{center}
	...
	\topcaption{Model parameters estimated from literature.}\label{tbl:model-parameters}
	\begin{scriptsize}
	\begin{supertabular}{c|l|l|l|l}
	\hline

	$R_{1}$ & RNA polymerase abundance & 75,000 & copies/cell & [REFHERE] \\
	$R_{2}$ & Ribosome abundance & 1 x 10$^{6}$ & copies/cell & [REFHERE] \\
	$G_{i}$ & Characteristic gene abundance & 2 & copies/cell & \\
	$K_{X}$ & Saturation constant transcription & 4,600 & copies/cell & \\
	$K_{T}$ & Saturation constant translation & 100,000 & copies/cell & \\
	& & & & \\
	$t_{1/2,m}$ & characteristic mRNA half-life & 2 & hr & [REFHERE] \\
	$t_{1/2,p}$ & characteristic protein half-life & 10 & hr & [REFHERE] \\
	...
	& & & & \\
	$e_{T}$ & elongation rate RNA polymerase & 6 & nt/s & [REFHERE] \\
	$e_{X}$ & elongation rate Ribosome & 5 & aa/s & [REFHERE] \\
	$L_{T,o}$ & characteristic gene length & 15,000 & nt & [REFHERE] \\
	$L_{X,o}$ & characteristic transcript length & 5,000 & nt & [REFHERE] \\
	$k_{T}$ & characteristic transcription rate & 1.44 & hr$^{-1}$ & derived \\
	$k_{X}$ & characteristic translation rate & 3.60 & hr$^{-1}$ & derived \\
	& & & & \\
	$D$	& Diameter of an HL-60 cell & 12 & $\mu$m$^{3}$ & [REFHERE] \\
	$f_{C}$ & cytoplasmic fraction & 0.51 & & [REFHERE] \\
